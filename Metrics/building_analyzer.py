...
models = [
    ('SFM', C_SFM, 'Building_SFM_run3'),
    ('RL', C_RL, 'Building_RL_run3'),
]
...
gs = gridspec.GridSpec(5, 3, hspace=0.42, wspace=0.35)

# (a) Evacuation Curve
ax = fig.add_subplot(gs[0, 0])
for name in data:
    ec = data[name]['ec']
    ax.plot(ec['time'], ec['evacuated_count'], color=colors[name], linewidth=2, label=name)
...
metrics_show = [
    ...
    ('Evac. rate', 'evacuation_rate', ''),
    ('Throughput', 'throughput_ps', 'p/s'),
    ('Mean speed', 'mean_speed_ms', 'm/s'),
    ('Mean headway', 'mean_headway_m', 'm'),
    ('Collisions AA', 'collisions_agent_agent', ''),
    ('Collisions AW', 'collisions_agent_wall', ''),
]
...
for label, key, unit in metrics_show:
    ...
    for name in data:
        ...
        row.append(f"{fmt(d.get(key, 'N/A'))} {unit}".strip())
    table_data.append(row)

col_labels = ['Metric'] + list(data.keys())
table = ax.table(cellText=table_data, colLabels=col_labels, cellLoc='center', loc='center', colWidths=[0.34] + [0.33] * len(data))
...
for i, name in enumerate(data):
    hm_pvs[name] = make_heatmap(fig.add_subplot(gs[3, i]),
                                 data[name]['hm'],
                                 f'({chr(106 + i)}) Density - {name}', 0, vmax)

# (l) Heatmap difference (RL - SFM)
ax = fig.add_subplot(gs[3, 2])
spv = hm_pvs['SFM']
rpv = hm_pvs['RL']
ci = spv.index.intersection(rpv.index)
cc = spv.columns.intersection(rpv.columns)
if len(ci) > 0 and len(cc) > 0:
    diff = rpv.reindex(index=ci, columns=cc).fillna(0) - spv.reindex(index=ci, columns=cc).fillna(0)
    ...
    ax.set_xlabel('X [m]')
    ax.set_ylabel('Z [m]')
    ax.set_title('(l) Difference (RL − SFM)')
...
ax.set_title('(m) Mean Headway Over Time')
ax.legend(fontsize=7)
ax.grid(True, alpha=0.3)
ax.set_xlim(0)

# (n) Collisions over time
ax = fig.add_subplot(gs[4, 1])
for name in data:
    co = data[name]['co']
    if len(co) > 0:
        co['time_bin'] = (co['time'] // 5) * 5
        hist = co.groupby(['time_bin', 'type']).size().unstack(fill_value=0)
        for col in hist.columns:
            ax.plot(hist.index, hist[col], color=colors[name], lw=1.5,
                    alpha=0.6 if col == 'Wall' else 1.0,
                    linestyle='--' if col == 'Wall' else '-',
                    label=f'{name} - {col}')
ax.set_xlabel('Time [s]')
ax.set_ylabel('Collisions per 5s')
ax.set_title('(n) Collisions Over Time')
ax.legend(fontsize=7)
ax.grid(True, alpha=0.3)
ax.set_xlim(0)
...
ax.set_xlabel('Local density [ped/m²]')
ax.set_ylabel('Specific flow [ped/(m·s)]')
ax.set_title('(o) Flow–Density Js(ρ)')
ax.legend(fontsize=8)
ax.grid(True, alpha=0.3)

plt.suptitle('Building — SFM vs RL', fontsize=14, fontweight='bold', y=1.0)
plt.savefig('./Charts/building_comparison.png', dpi=150, bbox_inches='tight')
print("\nDone")