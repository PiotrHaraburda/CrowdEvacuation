...
ax.axhline(y=total * 0.5, color='gray', linestyle=':', alpha=0.5, label='50%')
ax.axhline(y=total * 0.95, color='gray', linestyle='--', alpha=0.5, label='95%')
...
for name in data:
    ...
    w = max(1, len(ts) // 30)
    smooth = ts.rolling(window=w, center=True).mean()
    ax.plot(smooth.index, smooth.values, color=colors[name], linewidth=1.5, label=name)
ax.set_xlabel('Time [s]')
...
for name in data:
    ...
    w = max(1, len(ac) // 30)
    smooth = ac.rolling(window=w, center=True).mean()
    ax.plot(smooth.index, smooth.values, color=colors[name], linewidth=1.5, label=name)
ax.set_xlabel('Time [s]')
...
for name in data:
    ...
    w = max(1, len(th) // 20)
    smooth = th.rolling(window=w, center=True).mean()
    ax.plot(smooth.index, smooth.values, color=colors[name], linewidth=1.5, label=name)
ax.set_xlabel('Time [s]')
